...
export default function AssessmentDetails({
  ...
  return (
    <div className="space-y-4">
      {groups.length === 0 && <p className="text-sm text-zinc-500">Nenhuma medida registrada.</p>}

      {groups.map((g) => (
        <div key={g.category} className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800">
              ...
              return (
                <div key={m.id} className="flex items-baseline justify-between gap-3 py-1.5 text-sm">
                  <dt className="text-zinc-600 dark:text-zinc-400">
                    {m.label}
                    {m.is_calculated && <span className="text-xs text-zinc-500"> (calculado)</span>}
                  </dt>
      ...
      {(method || device || notes) && (
        <div className="space-y-1 rounded-xl border border-zinc-200 p-4 text-sm dark:border-zinc-800">
